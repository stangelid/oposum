...
function ModelBuilder:make_net()
  ...
  if opt.doc_att == 0 then
    local sent_cl = nn.Sequential()
    sent_cl:add(nn.Dropout(opt.dropout_p))
    sent_cl:add(nn.Linear(docencInputSize, opt.num_classes))
    if opt.cudnn == 1 then
      sent_cl:add(cudnn.SoftMax())
    else
      sent_cl:add(nn.SoftMax())
    end
    sent_cl:add(nn.Unsqueeze_nc(1,1))
    sent_cl.modules[2].name = 'linear'

    local par_cl = nn.Parallel(2,2)
    par_cl.name = 'par_cl'
    par_cl:add(sent_cl)

    for i=2,opt.max_doc do
      cloned_cl = sent_cl:sharedClone()
      cloned_cl.modules[2].name = 'lin_clone'
      par_cl:add(cloned_cl)
    end

    local head = nn.Sequential()
    head.name = 'head'
    head:add(nn.Mean(1,2))
    head:add(nn.Log())
    ...
    full_att:add(nn.Transpose({1,2}))
    full_att:add(biConcat)
